...
const alsoProjectNumberUpdate = (
    ...
    if (vs.length < 1) {
        ...
        handleCommunicationCellUpdate(submission, 'data.vodLMD', '');
        handleCommunicationCellUpdate(submission, 'data.buLMD', '');
        toast(
            <Toast
        ...
        const {
            ...
            country,
            code: companyCode,
        } = checkCountryPrefixEqual(value);
        if (
            !countryPrefixEqual &&
            props.rowData.data.invoiceTypeLMD === 'Invoice'
        ) {
            //Logic for case when country letters and prefix are different
            handleCommunicationCellUpdate(submission, 'data.vendorLMD', '');
            handleCommunicationCellUpdate(submission, 'data.buLMD', '');

            if (country === 'IS' || country === 'DE') {
                ...
                if (s.data.vendorName !== undefined) {
                    ...
                    if (currentVendor.includes('BU')) {
                        currentVendor = currentVendor
                            .toString()
                            .substring(0, currentVendor.toString().length - 7);
                    }
                    if (s.data.vendorName.includes('BU')) {
                        vendor = s.data.vendorName
                            ...
                            .substring(
                                0,
                                s.data.vendorName.toString().length - 7
                            );
            ...
            if (!valid) {
                ...
                toast(
                    <Toast
                        ...
                        message={'Vendor does not exist under this project'}
                        type={'error'}
                    />
                );
            }
